 ...
 *
 * Sets up SPI device handle, stores pin config and default radio config.
 * Must be called before any other DWM3000 functions.
 *
 ...
 *
 * Checks device ID, waits for IDLE, performs soft reset, reads OTP,
 * writes system config, and configures all registers.
 * Call after begin(), hard reset, checkSPI, IDLE wait, soft reset, IDLE wait.
 *
 * @param dev Pointer to DWM3000 device structure
 * @return esp_err_t ESP_OK on success
 */
