...
static uint32_t send_bytes(dwm3000_t *dev, uint8_t *data, int len,
  ...
  if (rec_len > 0) {
    ...
    // Combine received bytes into uint32_t
    uint32_t val = 0;
    for (int i = 0; i < rec_len; i++) {
      val |= ((uint32_t)rx_data[i]) << (8 * i);
    }
    return val;
  } else {
    // Transmit only
...
static uint32_t read_or_write_full_address(dwm3000_t *dev, uint32_t base,
  ...
  if (!is_write) {
    // Read operation
    // For DWM3000 read: Send Header, then send Dummy bytes while receiving Data
    // Total transaction length = Header Size + Data Length (4 bytes)
    size_t data_read_len = 4;
    size_t total_len = header_size + data_read_len;
    ...
    // Prepare Header in TX buffer
    if (header_size == 1) {
      tx_buf[0] = header & 0xFF;
    } else {
      tx_buf[0] = (header >> 8) & 0xFF;
      tx_buf[1] = header & 0xFF;
    }
    // Remainder of tx_buf is already 0 (dummy bytes)

    spi_transaction_t trans;
    memset(&trans, 0, sizeof(trans));
    trans.length = total_len * 8;
    trans.rxlength = total_len * 8; // In full-duplex, rx length should match
    ...
    if (ret != ESP_OK) {
      ...
      // Data bytes come after the header cycles
      // DWM3000 sends data while we send dummy bytes
      for (int i = 0; i < data_read_len; i++) {
        res |= ((uint32_t)rx_buf[header_size + i]) << (8 * i);
      }
    }
...
/**
 * @brief Initialize SPI device for DWM3000 (matches .ino begin())
 */
esp_err_t dwm3000_begin(dwm3000_t *dev, spi_host_device_t spi_host,
  ...
  dev->cs_pin = cs_pin;
  dev->rst_pin = rst_pin;
  dev->antenna_delay = 16350;

  // Set default configuration (matches .ino config[] array)
...
/**
 * @brief Full chip initialization (matches .ino init())
 */
esp_err_t dwm3000_chip_init(dwm3000_t *dev) {
  ...
  dwm3000_write(dev, 0x00, 0x3C, 0xFFFFFFFF, 0);
  dwm3000_write(dev, 0x00, 0x40, 0xFFFF, 0);
  dwm3000_write(dev, 0x0A, 0x00, 0x000900, 3);

  // RX and TX config registers
  dwm3000_write(dev, 0x3, 0x1C, 0x10000240, 0);
  dwm3000_write(dev, 0x3, 0x20, 0x1B6DA489, 0);
  dwm3000_write(dev, 0x3, 0x38, 0x0001C0FD, 0);
  dwm3000_write(dev, 0x3, 0x3C, 0x0001C43E, 0);
  dwm3000_write(dev, 0x3, 0x40, 0x0001C6BE, 0);
  dwm3000_write(dev, 0x3, 0x44, 0x0001C77E, 0);
  dwm3000_write(dev, 0x3, 0x48, 0x0001CF36, 0);
  dwm3000_write(dev, 0x3, 0x4C, 0x0001CFB5, 0);
  dwm3000_write(dev, 0x3, 0x50, 0x0001CFF5, 0);
  dwm3000_write(dev, 0x3, 0x18, 0xE5E5, 0);
  dwm3000_read(dev, 0x4, 0x20); // .ino: int f = read(0x4, 0x20)
  dwm3000_write(dev, 0x6, 0x0, 0x81101C, 0);
  dwm3000_write(dev, 0x07, 0x34, 0x4, 0);
  dwm3000_write(dev, 0x07, 0x48, 0x14, 0);
  dwm3000_write(dev, 0x07, 0x1A, 0x0E, 0);
  dwm3000_write(dev, 0x07, 0x1C, 0x1C071134, 0);
  dwm3000_write(dev, 0x09, 0x00, 0x1F3C, 0);
  dwm3000_write(dev, 0x09, 0x80, 0x81, 0);
...
void dwm3000_ds_send_frame(dwm3000_t *dev, int stage) {
  ...
  bool error = true;
  for (int i = 0; i < 50; i++) { // 50 retries matching .ino
  ...
  if (error) {
    ESP_LOGE(TAG, "[ERROR] Could not send frame successfully!");
  }
}

void dwm3000_ds_send_rt_info(dwm3000_t *dev, int t_roundB, int t_replyB) {
  dwm3000_set_mode(dev, 1);
  // .ino writes destination at 0x01 and sender at 0x02 (swapped vs sendFrame)
